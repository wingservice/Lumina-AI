...
// Fix: Restored the complete component logic, return statement, and default export to resolve "void" return type and "no default export" errors caused by truncation.
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (fbUser && !fbUser.emailVerified) {
        setError(
          <div className="text-center">
            Email not verified. <Link to={`/verify-email?email=${encodeURIComponent(email)}`} className="underline font-bold">Resend link?</Link>
          </div>
        );
        return;
      }
      ...
      setError(err.message === 'AUTH_FAILED' ? "Invalid email or password." : (err.message || "An unexpected error occurred."));
    } finally {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-indigo-950/20 via-zinc-950 to-zinc-950">
      <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="text-center mb-10">
          ...
          <p className="text-zinc-500 mt-2">Log in to your account to continue creating</p>
        </div>

        <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-8 backdrop-blur-xl space-y-6">
          {error && (
            <div className="p-4 bg-red-500/10 border border-red-500/20 text-red-500 text-xs rounded-xl font-bold">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5 ml-1">
                ...
                <Link to="/forgot-password" title="Forgot Password" className="text-[10px] font-bold text-indigo-400 hover:text-indigo-300">Forgot?</Link>
              </div>
              <input 
                type="password" 
                required
        ...
        <p className="text-center mt-8 text-sm text-zinc-500">
          ...
          <Link to="/signup" className="text-indigo-400 font-bold hover:text-indigo-300">Create one</Link>
        </p>
      </div>
    </div>
  );
